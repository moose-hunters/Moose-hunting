...
Game::Game()
    : m_window(nullptr), m_width(1024), m_height(768),
    m_cameraPos(0.0f, 1.5f, 8.0f), m_cameraFront(0.0f, 0.0f, -1.0f), m_cameraUp(0.0f, 1.0f, 0.0f),
    m_yaw(-90.0f), m_pitch(0.0f), m_lastX(512.0f), m_lastY(384.0f), m_firstMouse(true),
    m_fov(45.0f), m_lightPos(5.0f, 10.0f, 5.0f), m_lightColor(1.0f, 0.95f, 0.85f),
    m_floorTexture(0), m_floorY(1.5f), m_shootCooldown(0.0f), m_maxCooldown(0.5f), m_wasLMBPressed(false) {
}
...
bool Game::checkMooseHit() {
    const float MOOSE_HIT_RADIUS = 1.2f;
...
void Game::renderUI() {
    ...
    float startX = (m_width - totalW) / 2.0f;
    float topY = m_height - 30.0f;

    m_uiShader.setVec3("color", glm::vec3(1.0f, 0.2f, 0.2f));
